...
- 연구 방향: 심음 비지도학습 및 PCG 관련 연구입니다.

## Updated Idea Files
- `Idea_DataBase/Code_Guide_v1.py` (2026-03-23 14:26:02 KST)
  - 목적 요약: 비지도 심음 주기 분석을 위한 연구 코드를 작성하는 작업임입니다.
- `Idea_DataBase/Processed_Idea.py` (2026-03-23 14:26:02 KST)
  - 목적 요약: 비지도 심음 주기 분석용 전처리 단계를 구축. The script must load raw heart sound recordings and segmentation annotations, split recordings into heart cycles, extract a fixed-length numeric feature vector from each valid cycle, and save a clean feature table for later unsupervised learning입니다.
- `Idea_DataBase/Train_Idea.py` (2026-03-23 14:26:02 KST)
  - 목적 요약: 01_preprocess.py에서 생성한 주기 단위 수치형 feature matrix로 비지도 autoencoder를 학습. 각 심음 주기를 feature 공간에서 압축된 잠재 표현으로 학습하는 것입니다.
- `Idea_DataBase/Embedded_Idea.py` (2026-03-23 14:26:02 KST)
  - 목적 요약: 학습된 encoder와 전처리된 cycle feature matrix를 불러와 모든 valid cycle의 잠재 임베딩을 추출하고 latent space에서 HDBSCAN을 수행한 뒤 이후 해석에 필요한 결과를 저장입니다.
- `Idea_DataBase/Result_Idea.py` (2026-03-23 14:26:02 KST)
  - 목적 요약: HDBSCAN 군집 결과를 군집별 수치형 feature 분포 비교와 주요 구조 특성 파악을 통해 해석. 군집 라벨을 feature 공간에서 이해 가능한 심음 주기 패턴으로 해석하는 것입니다.
- `Idea_DataBase/Code_Guide_v2.py` (2026-03-23 14:26:02 KST)
  - 목적 요약: 현재 프로젝트의 비지도학습 파이프라인(전처리 → autoencoder 학습 → embedding 추출 및 HDBSCAN → cluster 해석)을 수정하고 싶습니다입니다.
- `Idea_DataBase/Code_Total_Result_Idea.py` (2026-03-23 14:26:02 KST)
...
- 핵심 목적: 비지도 심음 주기 분석을 위한 연구 코드를 작성하는 작업임입니다.
...
- 핵심 목적: 비지도 심음 주기 분석용 전처리 단계를 구축. The script must load raw heart sound recordings and segmentation annotations, split recordings into heart cycles, extract a fixed-length numeric feature vector from each valid cycle, and save a clean feature table for later unsupervised learning입니다.
- 목표: 비지도 심음 주기 분석용 전처리 단계를 구축. The script must load raw heart sound recordings and segmentation annotations, split recordings into heart cycles, extract a fixed-length numeric feature vector from each valid cycle, and save a clean feature table for later unsupervised learning입니다.
- 중요 맥락: 이 프로젝트는 원시 파형 시퀀스를 주된 모델 입력으로 사용하지 않음입니다.
- 중요 맥락: 주된 학습 입력은 각 cycle에서 추출한 수치형 feature vector임입니다.
- 중요 맥락: 하나의 심장 주기는 기본적으로 S1(i) 시작부터 다음 S1(i+1) 시작 전까지로 정의하며 그 사이에 S2(i)가 포함됨입니다.
- 중요 맥락: 샘플링 주파수는 4000 Hz로 고정함입니다.
- 중요 맥락: 기본적으로 리샘플링은 피함입니다.
- 핵심 제약: Do not invent the dataset schema입니다.
- 핵심 제약: Create a clearly marked adapter section where the user can map their actual files/columns into the expected internal schema입니다.
- 핵심 제약: If S1/S2 annotations are missing, raise a clear NotImplementedError instead of guessing입니다.
- 핵심 제약: If annotation format differs, isolate that logic in one adapter function입니다.
- 핵심 제약: Keep the code readable and heavily commented입니다.
- 분할 규칙: Validate cycle ordering입니다.
- 분할 규칙: Reject cycles with invalid order or clearly abnormal duration using configurable thresholds입니다.
- 분할 규칙: Save the reason for exclusion입니다.
- 분할 규칙: sample_id 정렬을 유지함 for all valid cycles입니다.
- 특징 추출: Implement a fixed-length numeric feature vector for each valid cycle입니다.
- 저장할 출력물: outputs/{RUN_NAME}/preprocess/cycle_features.npy입니다.
- 저장할 출력물: outputs/{RUN_NAME}/preprocess/cycle_metadata.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/preprocess/feature_names.json입니다.
- 저장할 출력물: outputs/{RUN_NAME}/preprocess/preprocess_summary.json입니다.
- 저장할 출력물: outputs/{RUN_NAME}/preprocess/feature_table.csv입니다.
- 메타데이터 필수 항목: sample_id입니다.
- 메타데이터 필수 항목: subject_id입니다.
- 메타데이터 필수 항목: recording_id입니다.
- 메타데이터 필수 항목: cycle_start_sample입니다.
- 메타데이터 필수 항목: cycle_end_sample입니다.
- 구현 가이드: Use numpy, pandas, scipy입니다.
- 구현 가이드: Keep the pipeline deterministic and configurable입니다.
- 구현 가이드: Write helper functions with docstrings and comments입니다.
- 구현 가이드: 라벨을 사용하지 않음입니다.
- 구현 가이드: Focus on creating a reliable tabular feature dataset for later unsupervised learning입니다.
- 완료 조건: the script runs as a standalone preprocessing script입니다.
- 완료 조건: valid cycles are segmented입니다.
- 완료 조건: one fixed-length feature vector is created per valid cycle입니다.
- 완료 조건: feature matrix and metadata are saved입니다.
- 완료 조건: exclusion reasons are tracked입니다.
...
- 핵심 목적: 01_preprocess.py에서 생성한 주기 단위 수치형 feature matrix로 비지도 autoencoder를 학습. 각 심음 주기를 feature 공간에서 압축된 잠재 표현으로 학습하는 것입니다.
- 목표: 01_preprocess.py에서 생성한 주기 단위 수치형 feature matrix로 비지도 autoencoder를 학습. 각 심음 주기를 feature 공간에서 압축된 잠재 표현으로 학습하는 것입니다.
- 중요 맥락: 이 단계는 원시 시계열 복원이 아니라 tabular feature 학습 작업임입니다.
- 중요 맥락: 모델 입력은 각 cycle의 고정 길이 수치형 feature vector임입니다.
- 중요 맥락: 원시 파형 텐서를 모델 입력으로 사용하지 않음입니다.
- 중요 맥락: 이 단계에서는 1D CNN, RNN, LSTM, Transformer 같은 시퀀스 모델을 사용하지 않음입니다.
- 중요 맥락: tabular 수치형 데이터에 맞는 단순 dense autoencoder를 사용함입니다.
- 필수 동작: 전처리 결과에서 valid cycle만 불러옴입니다.
- 필수 동작: sample 순서와 sample_id 정렬을 유지함입니다.
- 필수 동작: subject_id가 있으면 subject 단위로, 없으면 recording_id 단위로 train/validation을 분리함입니다.
- 필수 동작: validation의 정규화 통계가 training에 섞이지 않도록 함입니다.
- 모델 요구사항: Use TensorFlow/Keras입니다.
- 모델 요구사항: Build a simple dense autoencoder for tabular features입니다.
- 모델 요구사항: Make hidden layer sizes, activation, dropout, and latent dimension configurable입니다.
- 모델 요구사항: Normalize features in a reproducible way using training data only입니다.
- 모델 요구사항: Prefer a Keras Normalization layer adapted on training data only입니다.
- 학습 요구사항: Set random seed in a reproducible way입니다.
- 학습 요구사항: Use callbacks:입니다.
- 학습 요구사항: ModelCheckpoint입니다.
- 학습 요구사항: EarlyStopping with restore_best_weights=True입니다.
- 학습 요구사항: CSVLogger입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/cycle_features.npy입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/cycle_metadata.csv입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/feature_names.json입니다.
- 저장할 출력 파일: outputs/{RUN_NAME}/training/autoencoder.keras입니다.
- 저장할 출력 파일: outputs/{RUN_NAME}/training/encoder.keras입니다.
- 저장할 출력 파일: outputs/{RUN_NAME}/training/model_summary.txt입니다.
- 저장할 출력 파일: outputs/{RUN_NAME}/training/training_history.csv입니다.
- 저장할 출력 파일: outputs/{RUN_NAME}/training/training_summary.json입니다.
- 학습 요약 포함 항목: input dimension입니다.
- 학습 요약 포함 항목: number of training and validation samples입니다.
- 학습 요약 포함 항목: latent dimension입니다.
- 학습 요약 포함 항목: best validation loss입니다.
- 학습 요약 포함 항목: final training loss입니다.
- 구현 가이드: Add comments explaining feature matrix shape입니다.
- 구현 가이드: Keep code clear for a new lab member입니다.
- 구현 가이드: Fail loudly if feature matrix shape is invalid입니다.
- 구현 가이드: Do not perform clustering in this script입니다.
- 구현 가이드: Do not add unnecessary abstraction입니다.
- 완료 조건: the script trains end-to-end on the feature matrix입니다.
- 완료 조건: encoder and autoencoder are saved입니다.
- 완료 조건: training history is saved입니다.
- 완료 조건: reconstruction metrics are saved입니다.
- 완료 조건: outputs are ready for embedding extraction in the next stage입니다.

### Idea_DataBase/Embedded_Idea.py
- 핵심 목적: 학습된 encoder와 전처리된 cycle feature matrix를 불러와 모든 valid cycle의 잠재 임베딩을 추출하고 latent space에서 HDBSCAN을 수행한 뒤 이후 해석에 필요한 결과를 저장입니다.
- 목표: 학습된 encoder와 전처리된 cycle feature matrix를 불러와 모든 valid cycle의 잠재 임베딩을 추출하고 latent space에서 HDBSCAN을 수행한 뒤 이후 해석에 필요한 결과를 저장입니다.
- 중요 맥락: 이 단계는 tabular feature 임베딩을 대상으로 동작함입니다.
- 중요 맥락: 이 단계에서 모델을 다시 학습하지 않음입니다.
- 중요 맥락: 각 sample은 하나의 수치형 feature vector로 표현된 심음 cycle 1개에 대응함입니다.
- 중요 맥락: HDBSCAN은 잠재 feature 공간에서 안정적인 밀도 기반 군집을 찾아야 함입니다.
- 필수 동작: 전처리 단계와 동일한 sample 순서로 모든 valid cycle을 불러옴입니다.
- 필수 동작: sample마다 잠재 임베딩 벡터 1개를 추출함입니다.
- 필수 동작: sample_id 정렬을 유지함입니다.
- 필수 동작: 잠재 벡터에 HDBSCAN을 적용함입니다.
- 필수 동작: 군집 개수를 수동으로 고정하지 않음입니다.
- HDBSCAN 요구사항: Make key parameters configurable at the top:입니다.
- HDBSCAN 요구사항: min_cluster_size입니다.
- HDBSCAN 요구사항: min_samples입니다.
- HDBSCAN 요구사항: metric입니다.
- HDBSCAN 요구사항: cluster_selection_method입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/cycle_features.npy입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/cycle_metadata.csv입니다.
- 입력 파일: outputs/{RUN_NAME}/training/encoder.keras입니다.
- 저장할 출력물: outputs/{RUN_NAME}/clustering/embeddings.npy입니다.
- 저장할 출력물: outputs/{RUN_NAME}/clustering/embedding_metadata.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/clustering/cluster_assignments.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/clustering/hdbscan_model.joblib입니다.
- 저장할 출력물: outputs/{RUN_NAME}/clustering/clustering_summary.json입니다.
- 군집 할당 파일 필수 항목: sample_id입니다.
- 군집 할당 파일 필수 항목: subject_id입니다.
- 군집 할당 파일 필수 항목: recording_id입니다.
- 군집 할당 파일 필수 항목: cluster_label입니다.
- 군집 할당 파일 필수 항목: membership_probability if available입니다.
- 클러스터링 요약 필수 항목: total number of valid samples입니다.
- 클러스터링 요약 필수 항목: latent dimension입니다.
- 클러스터링 요약 필수 항목: number of clusters excluding noise입니다.
- 클러스터링 요약 필수 항목: number of noise samples입니다.
- 클러스터링 요약 필수 항목: 노이즈 비율입니다.
- 구현 가이드: Keep the code straightforward and reproducible입니다.
- 구현 가이드: Save enough information for the interpretation stage to join cluster assignments back to feature statistics and metadata입니다.
- 구현 가이드: 라벨을 사용하지 않음 or supervised evaluation입니다.
- 구현 가이드: Do not silently skip failures입니다.
- 완료 조건: embeddings are extracted for all valid samples입니다.
- 완료 조건: HDBSCAN labels are assigned입니다.
- 완료 조건: noise is explicitly reported입니다.
- 완료 조건: outputs are saved with sample_id alignment입니다.
- 완료 조건: basic visualization and summaries are generated입니다.
...
- 핵심 목적: HDBSCAN 군집 결과를 군집별 수치형 feature 분포 비교와 주요 구조 특성 파악을 통해 해석. 군집 라벨을 feature 공간에서 이해 가능한 심음 주기 패턴으로 해석하는 것입니다.
- 목표: HDBSCAN 군집 결과를 군집별 수치형 feature 분포 비교와 주요 구조 특성 파악을 통해 해석. 군집 라벨을 feature 공간에서 이해 가능한 심음 주기 패턴으로 해석하는 것입니다.
- 중요 맥락: 이 프로젝트는 비지도학습 기반임입니다.
- 중요 맥락: 해석은 추출된 수치형 feature vector를 중심으로 이루어져야 함입니다.
- 중요 맥락: 원시 파형 시각화에만 의존하지 않음입니다.
- 중요 맥락: 단일 수치 점수보다 군집 해석이 더 중요함입니다.
- 중요 맥락: 기본적으로 cluster_0, cluster_1, noise 같은 중립적인 군집 이름을 사용함입니다.
- 필수 분석: Cluster summary table입니다.
- 필수 분석: number of samples per cluster입니다.
- 필수 분석: proportion per cluster입니다.
- 필수 분석: noise proportion입니다.
- 필수 분석: Cluster-wise feature statistics입니다.
- 중요 제약: 명확한 근거가 없는 한 군집을 normal, murmur 같은 임상 라벨로 자동 명명하지 않으며, 필요 시에도 가설 수준으로만 제시함입니다.
- 중요 제약: 출력에서는 중립적인 표현을 사용함입니다.
- 중요 제약: 해석 코드는 재현 가능하고 검토하기 쉽게 유지함입니다.
- 중요 제약: sample_id 정렬을 유지함 in all merged tables입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/cycle_features.npy입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/cycle_metadata.csv입니다.
- 입력 파일: outputs/{RUN_NAME}/preprocess/feature_names.json입니다.
- 입력 파일: outputs/{RUN_NAME}/clustering/embeddings.npy입니다.
- 입력 파일: outputs/{RUN_NAME}/clustering/cluster_assignments.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/interpretation/cluster_summary.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/interpretation/cluster_feature_stats.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/interpretation/representative_samples.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/interpretation/noise_analysis.csv입니다.
- 저장할 출력물: outputs/{RUN_NAME}/interpretation/feature_difference_ranking.csv입니다.
- 마크다운 보고서 요약 항목: 발견된 군집 개수입니다.
- 마크다운 보고서 요약 항목: 노이즈 비율입니다.
- 마크다운 보고서 요약 항목: 군집 간 주요 특징 차이입니다.
- 마크다운 보고서 요약 항목: 각 군집을 특징짓는 feature 그룹입니다.
- 마크다운 보고서 요약 항목: 노이즈 그룹이 artifact 성향인지 또는 구조적으로 불안정한지 여부입니다.
- 구현 가이드: Use pandas, numpy, matplotlib입니다.
- 구현 가이드: Add comments explaining what each analysis block is testing입니다.
- 구현 가이드: Keep the code practical and readable입니다.
- 구현 가이드: 신호 복원보다 feature 기반 군집 해석 가능성에 초점을 둠입니다.
- 완료 조건: 군집 단위 테이블이 저장됨입니다.
- 완료 조건: feature 기반 비교 플롯이 저장됨입니다.
- 완료 조건: 대표 sample ID가 저장됨입니다.
- 완료 조건: 노이즈 분석 결과가 저장됨입니다.
- 완료 조건: 간단한 마크다운 해석 보고서가 생성됨입니다.
